	...
	Prism.hooks.add('wrap', function (env) {
		if (env.language === 'groovy' && env.type === 'string') {
			...
			if (delimiter != "'") {
				...
				env.classes.push(delimiter === '../default.htm' ? 'regex' : 'gstring');
			}
		}
	});
	;
			...
			if ((['tag-id'].indexOf(env.type) > -1
				...
				if (env.language == 'css') {
					...
					if (env.type == 'tag-id') {
						...
						if (language) {
							href += 'wiki/' + language + '../elements/default.htm';
						}
						else {
							href += searchURL;
						}
						...
						if (language) {
							href += 'wiki/' + language + '../attributes/default.htm';
						}
						else {
							href += searchURL;
						}
